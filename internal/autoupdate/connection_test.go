...
func TestConnect(t *testing.T) {
	...
	data, _, err := next(context.Background())
	if err != nil {
		t.Errorf("next(): %v", err)
	}
...
func TestConnectionReadNewData(t *testing.T) {
	...
	if _, _, err := next(context.Background()); err != nil {
		t.Errorf("next(): %v", err)
	}
...
func TestConnectionEmptyData(t *testing.T) {
	...
	t.Run("exit->not exist-> not exist", func(t *testing.T) {
		...
		isBlocking := blocking(func() {
			_, _, err = next(context.Background())
		})
...
func TestKeyNotRequestedAnymore(t *testing.T) {
	...
	if _, _, err := next(shutdownCtx); err != nil {
	...
	secondData, _, err := next(shutdownCtx)
	if err != nil {
...
func TestKeyRequestedAgain(t *testing.T) {
	...
	testData, _, err := next(shutdownCtx)
	if err != nil {
		t.Fatalf("Getting second data: %v", err)
	}
