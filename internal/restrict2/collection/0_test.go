...
func (tt testData) test(t *testing.T, f collection.FieldRestricter) {
	...
	t.Run(tt.name, func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("Calculate attribute funcs: %v", err)
		}

		requestUserAttr, err := attribute.NewUserAttributes(ctx, getter, tt.requestUserID)
		if err != nil {
			t.Fatalf("Calculate user attributes: %v", err)
		}

		allowed := make([]bool, len(tt.elementIDs))
		for i := range tt.elementIDs {
			allowed[i] = attrFuncs[i](requestUserAttr)
		}

		if tt.expect == nil {
			// test for one value

			if tt.expectOne != allowed[0] {
				t.Errorf("element %d %s, expected %s", tt.elementIDs[0], isAllowedString(allowed[0]), isAllowedString(tt.expectOne))
				return
			}
		...
		for i, a := range allowed {
			if a {
				allowedIDs = append(allowedIDs, tt.elementIDs[i])
			}
		}
...
func isAllowedString(b bool) string {
	if b {
		return "can be seen"
	}
...
func withPerms(meetingID int, perms ...perm.TPermission) testCaseOption {
	return func(td *testData) {
		...
		meetingGroupIDs := dskey.MustKey("meeting/%d/group_ids", meetingID)
